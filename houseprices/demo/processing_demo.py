...
# Import packages
import pandas as pd
from raymon import Trace, RaymonFileLogger, RaymonAPILogger, Tag, RaymonAPI
...
PROJECT_ID = os.environ.get("PROJECT_ID", "dda63fbf-ba25-4dee-b896-462542abb121")
RAYMON_URL = os.environ.get("RAYMON_ENDPOINT", "http://localhost:8000/v0")
LOG_PATH = os.environ.get("LOG_PATH", "/tmp/raymon")
SECRET = Path(os.environ.get("RAYMON_CLIENT_SECRET_FILE", ROOT / "m2mcreds-houseprices.json"))
USE_FILELOGGER = os.environ.get("USE_FILELOGGER", "False") == "True"
...
profile = load_profile(ROOT / "models/housepricescheap@2.0.0.json")
...
class HousePricingDeployment:
    def process(self, req_id, data, metadata):
        ...
        try:
            ...
            prep = self.prep.transform(data)
            trace.log(ref="preprocessed_input", data=rt.DataFrame(prep))
            # Model prediction
            pred_arr = self.model.predict(prep)
